...
def path_validate(map,possiblepath):
    orientation = [k for k,v in map.items() if v == '^' or v == '>' or v == 'v' or v == '<'][0]
    for step in possiblepath:
        if step == "R":
            if map[orientation] == '^':
                map[orientation] = '>'
            elif  map[orientation] == '>':
                map[orientation] = 'v'
            elif map[orientation] == 'v':
                map[orientation] = '<'
            elif map[orientation] == '<':
                map[orientation] = '^'
        elif step == "L":
            ...
            for x in range(0,int(step)):
                if map[orientation] == '^':
                    if map[(orientation[0],orientation[1]-1)] not in ['#','C']:
                        print(orientation)
                        return False
                    map[orientation] = '#'
                    map[(orientation[0],orientation[1]-1)] = '^'
                elif  map[orientation] == '>':
                    if map[(orientation[0]+1,orientation[1])] not in ['#','C']:
                        print(orientation)
                        return False
                    map[orientation] = '#'
                    map[(orientation[0]+1,orientation[1])] = '>'
                elif map[orientation] == 'v':
                    if map[(orientation[0],orientation[1]+1)] not in ['#','C']:
                        print(orientation)
                        return False
                    map[orientation] = '#'
                    map[(orientation[0],orientation[1]+1)] = 'v'
                elif map[orientation] == '<':
                    if map[(orientation[0]-1,orientation[1])] not in ['#','C'] :
                        print(orientation)
                        return False
                    ...
                    map[(orientation[0]-1,orientation[1])] = '<'            
                orientation = [k for k,v in map.items() if v == '^' or v == '>' or v == 'v' or v == '<'][0]
        if orientation == (24,4): return True
    return False

def print_map():
    buf = ''
    for y in range(0,1+max([k[1] for k,v in map.items()])):
        for x in range(0,1+max([k[0] for k,v in map.items()])):
            buf += map[(x,y)]
        buf += "\n"
    print(buf)
...
one_route = ["L","4","L","4","L","10","R","4","R","4","L","4","L","4","R","8","R","10","L","4","L","4","L","10","R","4","R","4","L","10","R","10","L","4","L","4","L","10","R","4","R","4","L","10","R","10","R","4","L","4","L","4","R","8","R","10","R","4","L","10","R","10","R","4","L","10","R","10","R","4","L","4","L","4","R","8","R","10"]

segments = set()
for x in range(0, len(one_route), 2):
    for x_ in range(2,10,2):
        segments.add(",".join(one_route[x:x+x_]))

print(len(segments))

combined_route = ",".join(one_route)

for a in segments:
    for b in segments:
        for c in segments:
            replaced_route = combined_route.replace(a, "A").replace(b, "B").replace(c, "C")
            if 'L' not in replaced_route and 'R' not in replaced_route:
                print(f'main:{replaced_route}')
                print(f'a:{a}')
                print(f'b:{a}')
                print(f'c:{a}')
