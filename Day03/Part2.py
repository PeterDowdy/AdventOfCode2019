...
def buildpath(path_nodes):
    nodes = []
    for path_node in path_nodes:
        direction = path_node[0]
        distance = int(path_node[1:])
        top_node = nodes[-1:]
        for n in range(0,distance):
            top_node = (0,0) if len(nodes) == 0 else nodes[-1:][0]
            if direction == 'R':
                top_node = (top_node[0]+1,top_node[1])
            if direction == 'U':
                top_node = (top_node[0],top_node[1]+1)
            if direction == 'L':
                top_node = (top_node[0]-1,top_node[1])
            if direction == 'D':
                top_node = (top_node[0],top_node[1]-1)
...
path1 = buildpath([x for x in testinput[0].split(',')])
path2 = buildpath([x for x in testinput[1].split(',')])

intersections = set(path1) & set(path2)

sorted_intersections = sorted(intersections, key = lambda x : signal_decay(x,path1,path2))
print(2+signal_decay(sorted_intersections[0],path1,path2))
